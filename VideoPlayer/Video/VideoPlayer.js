...
import PlayRateButtons from "../PlayRate/PlayRateButttons"
...
const VideoPlayer = ({ onVideoSelect }) => {
  ...
  const loadVideo = (quality) => {
    // Logic to load the video based on the selected quality
    console.log(`Loading video with quality: ${quality}`)
    // Add your video loading code here
  }
  ...
  const loadVideoWithQuality = (quality) => {
    // Perform the necessary action to load the video with the selected quality
    // For example, you can update the video source or set the quality settings of the video player
    // You'll need to replace the placeholder code with your actual implementation
    console.log(`Loading video with quality: ${quality}`)
  }
  ...
  const seekAmount = 3 // Seek amount in seconds
  const handleForward = () => {
  ...
  const handleVideoClick = (event) => {
    ...
    if (isSidebarElement) {
      return // Ignore click if it's within the RightSidebar component
    }
    const isControlBarElement = clickedElement.closest(".control_bar")
    if (isControlBarElement) {
      return // Ignore click if it's within the control bar
    }

    handlePlayPause() // Continue with play/pause logic for other areas of the video player
  }
          ...
          <div className="sidebar_container">
            <div className="sidebar">
              ...
              <SideBar
                ...
                onSubMenuSelect={handleSubMenuSelect} // Pass the callback function
              />
            </div>
          </div>
        )}
